...
class adminbaseclass
{
	 ...
	 function init(){  
	 	     ...
	 	     //modify by rain
	 	     $link = IUrl::creatUrlEx($mid, 'member/adminlogin');
	 	     if($this->admin['uid'] == 0 ||  $this->admin['mid'] != $mid) $this->message('member_nologin',$link);
			
	 	     $this->mid = $mid;
	 	     	
	 	     $data['admininfo'] = $this->admin; 
	 	     if($this->admin['groupid'] == 4){
	 	       $links = IUrl::creatUrlEx($this->mid,'areaadminpage/system'); 
	 	       $this->message('',$links);
	 	     }
