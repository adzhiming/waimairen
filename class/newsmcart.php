...
Class newsmcart{   
	...
	private $goodstype;
	private $shopid;
	private $defualt_struct =  array('mid'=>'0','goods'=>array(),'ggoods'=>array(),'shopg'=>array(),'shopgg'=>array()); //默认的购物车结构 
	...
	public function SetShopId($mid,$shopid){
		$this->mid = $mid;
		$this->shopid = $shopid;
		return $this;
	}
	...
	{ 
		...
		if($cartValue == null){
			...
			if($cartValue && isset($cartValue['mid']) && $cartValue['mid']==$this->mid) {
				return $cartValue;
			}
			return $this->defualt_struct;
		}
	...
	{ 
		if($goodsInfo) {
			if ($goodsInfo != $this->defualt_struct) {
				if($goodsInfo && empty($goodsInfo['mid'])) {
					$goodsInfo['mid'] = empty($this->mid)?'':$this->mid;
				}
	...
	public function ShopList(){
		if($this->init()){   
			...
			foreach($shopids as $key=>$value){
				// 完整内容 	 
				if($this->SetShopId($mid,$value)->OneShop()){
