...
class drawbacklog
{
	 ...
	 *  @details Details
	 */
	function __construct($mid,$mysql,$usercls,$appid){
	...
	if($this->drawdata  == null ){
		...
		if($orderinfo['buyeruid'] != $memberinfo['uid']){
				$this->error = '购买用户和用户不一致'.$orderinfo['buyeruid'].' '.$memberinfo['uid'] ;
				return false;
		}
...
/*		 if(empty($data['content'])){
	...
	*/
	...
	private function GetMem(){
		return $this->actionclas->getinfo($this->mid,$this->appid);
	}
	//放回 管理员信息
	private function GetAdmin(){
		return $this->actionclas->getadmininfo($this->mid);
	}  
}
?>