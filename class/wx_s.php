...
class wx_s
{
   ...
   '40054'=>'invalid sub button url domain');
	 ...
	 //  失败     {"errcode":40018,"errmsg":"invalid button name size"} 
	  function __construct(){ 	  
	  	...
	  	$this->mid =  Mysite::$app->config['mid'];
    }
    //获取token
   function checktoken(){

      $config = new config('autorun.php',hopedir,$this->mid,$type=1,Mysite::$app->config['conf_table']);   
   ...
   function sendmsg($mid, $msg,$useropenid){
...

{
  ...
  public function getSignPackage($mid) {
    $params = array ();
    $params ['mid'] = $mid;    
    if (api_post_getCardJSApiTicket ( $params, $return_code, $return_msg, $return_data )) {
    	if ($return_code == 1 && $return_data) {
    		$jsapiTicket = $return_data['ticket'];
