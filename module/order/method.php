...
//	   $link = IUrl::creatUrlEx($this->mid,'site/waitpay/orderid/'.$orderid);
   ...
   function setstatus(){
	...
	function usersorder(){
	  ...
	  $data['actiondo'] = 'order';
		...
		$link = IUrl::creatUrlEx($this->mid,'member/order/status/'.$status.'/stime/'.date('Y-m-d',$stime).'/etime/'.date('Y-m-d',$etime).'/orderdatediff/'.$orderdatediff.'/page/@page@');
	...
	function usermorder(){
		...
		$data['orderdatediff'] = $orderdatediff;
			$status = empty($status)?5:$status;
		$link = IUrl::creatUrlEx($this->mid,'order/usermorder/status/'.$status.'/stime/'.date('Y-m-d',$stime).'/etime/'.date('Y-m-d',$etime).'/orderdatediff/'.$orderdatediff.'/page/@page@');
	...
	function userunorder(){ 
		...
		//预定订单取消
		if($orderinfo['is_goshop'] == 2){
			$res['ydstatus'] = 3;
			$this->mysql->update(Mysite::$app->config['tablepre'].'order',$res,"id =".$orderinfo['id']."");
			$ordercontrol->buyerunorder($this->member['uid']);
	...
	function userdelorder(){  // 用户删除订单
		...
		$orderid = intval(IReq::get('orderid'));
		$orderinfo  = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."order where id='".$orderid."'  ");
		if($orderinfo['mid'] !=$this->mid)  $this->message('不合法的访问');
