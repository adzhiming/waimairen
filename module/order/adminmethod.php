...
{
	function index(){
	 	    	 $link = IUrl::creatUrlEx($this->mid,'adminpage/order/module/orderlist');
   ...
   function orderlist(){
        ...
        $mid = $this->mid;
	    	...
	    	$endtime = empty($endtime)? $nowday:$endtime;
	     	$where = "  where ord.mid='".$this->mid."'".' and ord.addtime > '.strtotime($starttime.' 00:00:00').' and ord.addtime < '.strtotime($endtime.' 23:59:59');
	     ...
	     $data['orderstatus'] = '';
	    	...
	    	//$link = IUrl::creatUrlEx($this->mid,'/adminpage/order/module/orderlist'.$newlink);
	    	$link = IUrl::creatUrlEx($this->mid, '/adminpage/order/module/orderlist'.$newlink);
	    	$pageshow = new page();
	...
	function ordertoday(){
		...
		$dno = IReq::get('dno');
		$data['dno'] = $dno=="showdet" || empty($dno)?$dno="0":$dno;
...
//statustype  4   退款处理
		...
		$this->setstatus();
		$nowday = date('Y-m-d',time());
	  $where = "  where ord.mid='".$this->mid."'".' and ord.addtime > '.strtotime($nowday.' 00:00:00').' and ord.addtime < '.strtotime($nowday.' 23:59:59');
		//查询当天所有订单数据

	 //	$where .= ' and ord.status = 0 ';
	  if(!empty($shopid)){
	      $where .= " and ord.shopid='".$shopid."' ";
	  }
	  $where .= $statustypearr[$statustype];
		//$where .= ' and ord.status = 0 ';
	  $where .= empty($dno) || $dno=="0"?'':' and ord.dno =\''.$dno.'\'';
	  ...
	  if($orderlist)
	  {
		  foreach($orderlist as $key=>$value)
		  {
	       ...
	       /*构造店铺*/
	        ...
	        $shopinfo = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where mid='".$this->mid."'  order by id desc");
	   
	 	 $data['shoplist'] = $shopinfo;
...
//		              }
   ...
   function setstatus(){
		   ...
		   $paytypelist = array(0=>'到店支付',1=>'在线支付','weixin'=>'微信支付');
	...
	function ordercomment(){
	   ...
	   if(!empty($querytype))
	   {
	   	 if(!empty($searchvalue))
	   	 {
	   	   $data['searchvalue'] = $searchvalue;
	   	   $where .= ' and '.$querytype.' LIKE \'%'.$searchvalue.'%\' ';
	   	   $newlink = IUrl::creatUrlEx($this->mid,'adminpage/order/module/ordercomment/'.$searchvalue.'/querytype/'.$querytype);
	   	   $data['querytype'] = $querytype;
	   	 }
	   }
	  $pageinfo = new page();
    ...
    //orderdet: id  order_id  goodsid  goodsname  goodscost  goodscount  status  shopid
		...
		foreach($list as  $key=>$value){
				...
				if( empty($goodsinfo) ){
					...
					$value['goodsname'] = $goodsinfo['goodsname'].'【'.$goodsinfo['attrname'].'】'.'【刷】';
				}else{
  ...
  function autodel(){
      ...
      $link = IUrl::creatUrlEx($this->mid,'adminpage/order/module/orderlist');//sendtime 发货时间
	 	  $this->message('',$link);
	}
  ...
  function drawbacklog(){
	    	...
	    	$starttime = empty($starttime)? $nowday:$starttime;
	    	$endtime = empty($endtime)? $nowday:$endtime;
	      $where = "  where mid='".$this->mid."'".' and addtime > '.strtotime($starttime.' 00:00:00').' and addtime < '.strtotime($endtime.' 23:59:59');
	    	$data['starttime'] = $starttime;
	    	$data['endtime'] = $endtime;
	    	$newlink = '/starttime/'.$starttime.'/endtime/'.$endtime;
	    	 $data['searchvalue'] ='';
	     ...
	     $data['orderstatus'] = '';
	    	...
	    	$link =  IUrl::creatUrlEx($this->mid,'/adminpage/order/module/drawbacklog'.$newlink);
	    	$pageshow = new page();
	    	$pageshow->setpage(IReq::get('page'),5);
	    	 //order: id  dno 订单编号 shopuid 店铺UID shopid 店铺ID shopname 店铺名称 shopphone 店铺电话 shopaddress 店铺地址 buyeruid 购买用户ID，0未注册用户 buyername
	    	 //
  ...
  function showdrawbacklog(){
  	error_reporting(0);
     $id = IFilter::act(IReq::get('id'));
     $link = IUrl::creatUrlEx($this->mid,'adminpage/order/module/drawbacklog');
  ...
  function showcommlist(){ 
	...
	function shophuiorder(){
        $this->setstatus();
	     ...
	     $data['orderstatus'] = '';
	    	...
	    	$link = IUrl::creatUrlEx($this->mid,'/adminpage/order/module/shophuiorder'.$newlink);
	    	$pageshow = new page();
	    	$pageshow->setpage(IReq::get('page'),5);
	    	 //order: id  dno 订单编号 shopuid 店铺UID shopid 店铺ID shopname 店铺名称 shopphone 店铺电话 shopaddress 店铺地址 buyeruid 购买用户ID，0未注册用户 buyername
	    	 //
	...
	function shophuiautodel(){
      ...
      $link = IUrl::creatUrlEx($this->mid,'adminpage/order/module/shophuiorder');
	 	  $this->message('',$link);
	}
