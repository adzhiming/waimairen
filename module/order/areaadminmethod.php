...
{
	function index(){
	 	    	 $link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/orderlist');
        ...
        $this->setstatus();
	      ...
	      $where = '  where ord.admin_id = '.$admin_id.' and  ord.addtime > '.strtotime($starttime.' 00:00:00').' and ord.addtime < '.strtotime($endtime.' 23:59:59');
	    	...
	    	if(!empty($querytype))
	    	{
	    		...
	    		}//IUrl::creatUrlEx($this->mid,'admin/asklist/commentlist
	    	}
	     $data['orderstatus'] = '';
	    	...
	    	$link = IUrl::creatUrlEx($this->mid,'/areaadminpage/order/module/orderlist'.$newlink);
	    	$pageshow = new page();
...
//statustype  4   退款处理
		...
		$this->setstatus();
		$nowday = date('Y-m-d',time());
 	  $where = '  where  ord.admin_id = '.$this->admin['uid'].' and  ord.addtime > '.strtotime($nowday.' 00:00:00').' and ord.addtime < '.strtotime($nowday.' 23:59:59');
		//查询当天所有订单数据
	...
	function ordertotal()
	{
		...
		$starttime = empty($starttime)? $nowday:$starttime;
		$endtime = empty($endtime)? $nowday:$endtime;
	  $where = '  where ord.admin_id = '.$this->admin['uid'].' and   ord.posttime > '.strtotime($starttime.' 00:00:00').' and ord.posttime < '.strtotime($endtime.' 23:59:59');
		$data['starttime'] = $starttime;
		$data['endtime'] = $endtime;
		$data['querytype'] = '';
		$data['searchvalue'] = '';
	...
	function orderyjin()
	{
  	 ...
  	 if(!empty($admin_id)){
		...
		$link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/orderyjin'.$newlink);
	  $data['outlink'] =IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outtjorder/outtype/query'.$newlink);
	  $data['outlinkch'] =IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outtjorder'.$newlink);
		$pageinfo = new page();
		...
		if(is_array($shoplist))
		{
	     foreach($shoplist as $key=>$value)
	     {
			 ...
			 if( $value['yjin'] == 0 ||   $value['yjin'] == '0.00' ||  $value['yjin'] == ''  ){
		       ...
		       $value['outdetail'] =IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outdetail/outtype/query/shopid/'.$value['id'].$newlink);
		       $list[] = $value;
		   }
		}
		...
		switch($type){
			  ...
			  if($orderinfo['status'] > 3){
			   ...
			   if($orderinfo['paytype_name'] == 'open_acout'){
					...
					$this->memberCls->addmemcostlog($this->mid,  $orderinfo['buyeruid'],$memberinfo['username'],$memberinfo['cost'],1,$zengcost,$shengyucost,$bdliyou,ICookie::get('adminuid'),ICookie::get('adminname') );
...
//					}
	...
	function ordercomment(){
	   ...
	   if(!empty($querytype))
	   {
	   	 if(!empty($searchvalue))
	   	 {
	   	   ...
	   	   $newlink = IUrl::creatUrlEx($this->mid,'areaareaadminpage/order/module/ordercomment/'.$searchvalue.'/querytype/'.$querytype);
	   	   $data['querytype'] = $querytype;
	   	 }
	   }
	  $pageinfo = new page();
  ...
  function autodel(){
      ...
      $link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/orderlist');//sendtime 发货时间
	 	  $this->message('',$link);
	}
  ...
  function drawbacklog(){
	     ...
	     $data['orderstatus'] = '';
	    	...
	    	$link = IUrl::creatUrlEx($this->mid,'/areaadminpage/order/module/drawbacklog'.$newlink);
	    	$pageshow = new page();
	    	$pageshow->setpage(IReq::get('page'),5);
	    	 //order: id  dno 订单编号 shopuid 店铺UID shopid 店铺ID shopname 店铺名称 shopphone 店铺电话 shopaddress 店铺地址 buyeruid 购买用户ID，0未注册用户 buyername
	    	 //
  ...
  function showdrawbacklog(){
     $id = IFilter::act(IReq::get('id'));
     $link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/drawbacklog');
  ...
  function showcommlist(){ 
     ...
     $searchvalue = IReq::get('searchvalue');
		...
		$link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outareatjorder'.$newlink);
	  $data['outlink'] =IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outareatjorder/outtype/query'.$newlink);
	  $data['outlinkch'] =IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/outareatjorder'.$newlink);
		$pageinfo = new page();
		$pageinfo->setpage(IReq::get('page')); 
   ...
   function outareatjorder()
	...
	function shophuiorder(){
        $this->setstatus();
	     ...
	     $data['orderstatus'] = '';
	    	...
	    	$link = IUrl::creatUrlEx($this->mid,'/areaadminpage/order/module/shophuiorder'.$newlink);
	    	$pageshow = new page();
	    	$pageshow->setpage(IReq::get('page'),5);
	    	 //order: id  dno 订单编号 shopuid 店铺UID shopid 店铺ID shopname 店铺名称 shopphone 店铺电话 shopaddress 店铺地址 buyeruid 购买用户ID，0未注册用户 buyername
	    	 //
	...
	function shophuiautodel(){
      ...
      $link = IUrl::creatUrlEx($this->mid,'areaadminpage/order/module/shophuiorder');
	 	  $this->message('',$link);
	}
