...
class method   extends adminbaseclass
{
	...
	function saveshop()
	{
	 ...
	 function shopbiaoqian()
	 {
	 	  ...
	 	  $attrinfo = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shoptype where  cattype='".$shopinfo['shoptype']."' and  parent_id = 0 and is_admin = 1 and mid='".$mid."' order by orderid desc limit 0,1000");//获取所有后台控制属性
	...
	function resetdefualt(){
	  ...
	  ICookie::set('adminshopid',$shopid,86400);
		$link = IUrl::creatUrlEx($this->mid,'shopcenter/index');
	...
	function savelunadv(){
	    ...
	    $links = IUrl::creatUrlEx($this->mid,'shop/shoplunadv');
	...
	function savegoodsall(){
		...
		$link = IUrl::creatUrlEx($this->mid,'adminpage/shop/module/goodslibrary'); 
		...
		$goodsinfo['typeid'] = $data['typeid']; 
                //$this->success('更新成功！');
		echo "<script>alert('更新成功！');parent.refreshgoods(".json_encode($goodsinfo).");</script>";
		exit; 
	...
	function selectmarketimg(){
		...
		$data['imglist'] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."imglist      order by addtime desc limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()." ");
	  	...
	  	$link = IUrl::creatUrlEx($this->mid,'adminpage/shop/module/selectmarketimg/gid/'.$data['gid']);//index.php?ctrl=&action=&module=
	...
	function showimglist(){ 
		...
		$data['imglist'] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."imglist      order by addtime desc limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()." ");
	  	...
	  	$link = IUrl::creatUrlEx($this->mid,'adminpage/shop/module/showimglist');//index.php?ctrl=&action=&module=
	...
	function goodsgg(){ 
		$pageshow = new page();
	    	...
	    	$mid = $this->mid;
	    	$templist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."goodsgg where parent_id =0 and mid ='".$mid."' order by orderid asc limit ".$pageshow->startnum().", ".$pageshow->getsize()."");
	    	$shuliang  = $this->mysql->counts("select * from ".Mysite::$app->config['tablepre']."goodsgg  where parent_id = 0  and mid ='".$mid."' ");
	...
	function editgoodsgg(){
		...
		$id = intval(IReq::get('id'));
		$data['gginfo']['mid'] = $mid;
		$data['gginfo'] = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."goodsgg where id =".$id." and parent_id = 0  and mid ='".$mid."' order by orderid asc limit 0,30");
		$data['ggdet'] = array();
	...
	function savemaingg(){
		...
		$link = IUrl::creatUrlEx($this->mid,'adminpage/shop/module/editgoodsgg/id/'.$id);
	...
	function delgoodschildgg(){
 	...
 	function searchattr(){
		$mid = $this->mid;
		$templist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop_searkey where mid ='".$mid."' ");		
		$data['keywordlist'] = $templist;
		Mysite::$app->setdata($data);
	}
function savesearchwords(){   //向配置文件中保存搜索关键词
		...
		$datalist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop_searkey where mid ='".$mid."' ");    
		$i=0;
		$k=count($typename);
		$j=count($datalist);
		
		for($i=0;$i<$k;$i++){
			$data['keyword'] = $typename[$i];
			if($i>$j){ 
				$data['mid'] = $mid;
				$this->mysql->insert(Mysite::$app->config['tablepre'].'shop_searkey',$data);
			}
			else{
				//var_dump($data."insert");
				$this->mysql->update(Mysite::$app->config['tablepre'].'shop_searkey',$data,"id='".$datalist[$i]['id']."' ");
			}
		}
