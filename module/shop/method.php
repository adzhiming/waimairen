...
class method   extends baseclass
{
	...
	//店铺总销量
	 ...
	 function showgoods(){  	//菜品详情
		...
		$newshoparray = array_merge($data['shopinfo'],$shopdet);
      	 	 $tempinfo =   $this->pscost($newshoparray,1,0); 
 ...
 function showgoodszzz(){		//展示商品详情
      ...
      if(empty($goodsinfo)){
      	$link = IUrl::creatUrlEx($this->mid,'shop/index'); 
...
//	 if($checkps['canps'] != 1) $this->message('shop_notinps');  
  ...
  function savepxmyF(){
		  ...
		  if(empty($checkshop)){
		  	...
		  	if(empty($checkshop2)){
		  	  $this->message('shop_noexit');
		  			...
		  			$link = IUrl::creatUrlEx($this->mid,'market/shopshow/id/'.$collectid);
		    $this->message('',$link); 
		  	}
		  }else{
		  	$link = IUrl::creatUrlEx($this->mid,'shop/index/id/'.$collectid);
		    $this->message('',$link);
		  } 
	}
	...
	function openshop(){
		if(empty($this->member['uid'])){
			  $link = IUrl::creatUrlEx($this->mid,'member/index');
	...
	function mangeshop(){
	    ...
	    $link =  IUrl::creatUrlEx($this->mid,'shopcenter/useredit'); ///http://192.168.0.104/index.php?ctrl=&action=;
