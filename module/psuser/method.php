   ...
   function sendorder(){
    ...
    function ordercontrol(){
  	 ...
  	 if($dotype == 'joinorder'){//加入订单
			...
			if($ordercheck['status'] > 1){//当订单状态大于1时删除 配送单状态小于1的订单
				$checka = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."orderps where orderid='".$orderid."' and status < 2 "); 
				if(!empty($checka)){
					$this->mysql->delete(Mysite::$app->config['tablepre']."orderps"," orderid='".$orderid."'  and status <2  "); 
				}
			} 
			if($ordercheck['psuid']> 0){   
				$this->mysql->delete(Mysite::$app->config['tablepre']."orderps"," orderid='".$orderid."'  and psuid !=".$ordercheck['psuid']."  ");  
				$this->message('订单已被抢'); 
