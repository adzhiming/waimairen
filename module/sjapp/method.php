	...
	*/
	function goodslist(){
		...
		foreach ($goodslist as &$value) {
			$value["img"] = $value["img"]?Mysite::$app->config['imgserver'].$value["img"]:"";
		}
...
';
	...
	function getgoods(){
		...
		$this->pageCls->setpage($page, 10); 
					$goodsdet = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."goods where typeid = ".$typeid."  and shopid =".$shopid."  order by id asc limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
					$temparr = array();
					foreach($goodsdet as $k=>$v){
						$cxinfo = $this->goodscx($v);
						$v['is_cx'] = $cxinfo['is_cx'];
						...
						if($v['have_det'] ==1){
							$v['product'] = $this->mysql->getarr("select id,attrname,attrids,stock,cost  from ".Mysite::$app->config['tablepre']."product where goodsid = ".$v['id']."  and shopid =".$shopid."  order by id asc");
						}else{
							$v['product'] = array(); 
						}
	...
	function newshop(){
		...
		*/
		...
		$cxrule = $this->mysql->getarr("select name,signid  from ".Mysite::$app->config['tablepre']."rule  where shopid = '".$value["id"]."' and starttime < ".time()."	and endtime  > ".time()."  and  status =1   ");
		$cxinfo = array();
		foreach($cxrule as $key=>$value2){
			$value2['logo'] = isset($ruleimg[$value2['signid']])?Mysite::$app->config['siteurl'].$ruleimg[$value2['signid']]:'';
			$cxinfo[] = $value2;
		}
			$value['cxinfo'] = $cxinfo;//$temprule;
			if($juli > 1000){
				$juli = $juli*0.001;
				$juli = round($juli,2);
		...
		foreach($list as $key=>$value){  
			...
			if(isset($delvalue['shopid'])){ 
					...
					// foreach($rulist  as $k=>$v){
						// $temprulelist[$v['controltype']][] = $v['name'];
					// }
		$cxrule = $this->mysql->getarr("select name,signid  from ".Mysite::$app->config['tablepre']."rule  where shopid = '".$cvalue["id"]."' and starttime < ".time()."	and endtime  > ".time()."  and  status =1   ");
		$cxinfo = array();
		foreach($cxrule as $key=>$value2){
			$value2['logo'] = isset($ruleimg[$value2['signid']])?Mysite::$app->config['siteurl'].$ruleimg[$value2['signid']]:'';
			$cxinfo[] = $value2;
		}
			$cvalue['cxinfo'] = $cxinfo;//$temprule;
...
/*					$sellrule->setdata($cvalue['id'],1000,$cvalue['shoptype']);
	...
	function getcx(){
		...
		$valuelist = empty($shop['pradiusvalue'])? unserialize(Mysite::$app->config['radiusvalue']):unserialize($shop['pradiusvalue']);
		$juliceshi = intval($juli/1000);
		$backdata['is_onlinepay'] = $shopinfo['is_onlinepay'];
		$backdata['is_daopay'] = $shopinfo['is_daopay'];
		if(is_array($valuelist)){
			foreach($valuelist as $k=>$v){
				if($juliceshi == $k){
  	       	        $shop['baidupscost'] = $v;
	  ...
	  //获取所有促销 
		...
		$backdata['yhjlist']  = $this->mysql->getarr("select id,name,cost from ".Mysite::$app->config['tablepre']."juan  where uid ='".$backinfo['uid']."'  and status = 1 and endtime > ".time()." and limitcost < ".$backdata['surecost']."  order by id desc limit 0,20");
		$this->success($backdata);
	}
	...
	function sendregphone(){
		...
		if($phonecls->sendcode()){
			$this->success($phonecls->getLimittime());//$phonecls->getCode()
		}else{
			$this->message($phonecls->getError());
		} 
	}
	...
	//	$enddata = date('Y-m-d', strtotime("$BeginDate +1 month -1 day"));
		...
		$tj =  $this->mysql->select_one("select  count(id) as shuliang,sum(cxcost) as cxcost,sum(yhjcost) as yhcost, sum(shopcost) as shopcost,sum(scoredown) as score, sum(shopps)as pscost, sum(bagcost) as bagcost,sum(allcost) as doallcost from ".Mysite::$app->config['tablepre']."order  where shopuid = '".$backinfo['uid']."'  and   status < 3 ".$where2."  order by id asc  limit 0,1000");
		 $data['ordernum'] = intval($tj['shuliang']); //订单数
		$data['ordercost'] = round($tj['doallcost'],2);//总金额
	...
	//	print_r($orderinfo);
		...
		if($orderinfo['status']  > 2){
				...
				$errdata['url'] = $payerrlink;
				$errdata['reason'] = '此订单已发货或者其他状态不可操���';
				$errdata['paysure'] = false;  
				$this->showpayhtml($errdata);  
...
/* 		$newdata['bagcost'] = intval(IFilter::act(IReq::get('bagcost')));
	...
	function getnewconfig(){
		...
		$config = new config('appset.php',hopedir,$this->mid,$type=1,'system_config');   
   ...
   function addresslist(){
	...
	function apppaydata(){
		...
		if($type == 'acount'){//余额充值 
			...
			$titlename = "在线充值";
			if(!empty($alipaychek)){
				$dopaydata = array('mid'=>$this->mid,'type'=>'acount','upid'=>$backinfo['uid'],'cost'=>$cost,'source'=>0,'status'=>0,'addtime'=>time());//支付数据  
			...
			$allcost = $orderinfo['allcost'];
			if(!empty($alipaychek)){  
				$dopaydata = array('mid'=>$this->mid,'type'=>'order','upid'=>$orderid,'cost'=>$orderinfo['allcost'],'source'=>0,'status'=>0,'addtime'=>time());//支付数据 
				$this->mysql->insert(Mysite::$app->config['tablepre'].'onlinelog',$dopaydata);
				$newid = $this->mysql->insertid(); 
	...
	function appacoutpay(){
	    ...
	    if($type == 'order'){//支付订单 
			...
			$this->memberCls->addmemcostlog($this->mid,$orderinfo['buyeruid'],$backinfo['username'],$backinfo['cost'],2,$orderinfo['allcost'],$accost,"下单余额消费",$backinfo['uid'],$backinfo['username']);
			$orderCLs = new orderclass(); 
            			 	   ...
            			 	   $attra['input'] =  $attra['input'] > 0?$attra['input']:$vall['id'];
            			 	  	...
            			 	  	$attra['checkbox'] =  $attra['checkbox'] > 0?$attra['checkbox']:$vall['id'];
									...
									if($zongpointcount != 0 ){
            			    			...
            			    			$values['shoplogo'] = empty($values['shoplogo'])? Mysite::$app->config['imgserver'].Mysite::$app->config['shoplogo']:$values['shoplogo'];
...
if($showtype == 1){    // 加载商品
	...
	public function duiba(){
		...
		$duibaTool = new DuiBaTool();
		$url = $duibaTool->buildCreditAutoLoginRequest(Mysite::$app->config['duiba_appkey'],Mysite::$app->config['duiba_secret'],$backinfo['uid'],$backinfo['score']);
		
		header("location:".$url);
		exit;
	}
	
	function duibaConsume(){
		$duibaTool = new DuiBaTool();
		$duibaTool->parseCreditConsume(Mysite::$app->config['duiba_appkey'],Mysite::$app->config['duiba_secret'], $_REQUEST);
		
		$credits = intval($_REQUEST["credits"]);
		
		$member = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."member where uid ='".$_REQUEST["uid"]."' and status = 0");
		
		if(!$member){
			echo json_encode(array(
				"status"=>"fail",
				"errorMessage"=>"用户不存在或账号不可用",
				"credits"=>"0"
				));
			exit;
		}
		
		if(intval($member["score"]) < $credits){
			echo json_encode(array(
				"status"=>"fail",
				"errorMessage"=>"您的积分不足",
				"credits"=>$member["score"]
				));
			exit;
		}
		
	   	 $ndata['score'] = $member['score'] - $credits;
	   	 //更新用户积分
	     $this->mysql->update(Mysite::$app->config['tablepre'].'member',$ndata,"uid='".$member['uid']."'");
		...
		//写log
	   	...
	   	$data['ip'] = $_REQUEST['ip'];
	   	$data['waitAudit'] = $_REQUEST['waitAudit'];
	   	$data['params'] = $_REQUEST['params'];
	   	$data['sign'] = $_REQUEST['sign'];
		$this->mysql->insert(Mysite::$app->config['tablepre'].'duibalog',$data);
	...
	function duibaNotify(){
		$duibaTool = new DuiBaTool();
		$ret = $duibaTool->parseCreditNotify(Mysite::$app->config['duiba_appkey'],Mysite::$app->config['duiba_secret'], $_REQUEST);
		$log = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."duibalog where orderNum ='".$ret["bizId"]."'");
		...
		$member = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."member where uid ='".$log["uid"]."' and status = 0");
		
		
		
		if(!$ret["success"]){
			//退还积分
			$ndata['score'] = $member['score'] + $log["credits"];
	   	 	//更新用户积分
	     	$this->mysql->update(Mysite::$app->config['tablepre'].'member',$ndata,"uid='".$member['uid']."'");
	     	
	     	
	     	$this->mysql->update(Mysite::$app->config['tablepre'].'duibalog',array("status"=>-1),"orderNum='".$log['orderNum']."'");
		}
		
		
	    $this->mysql->update(Mysite::$app->config['tablepre'].'duibalog',array("status"=>1),"orderNum='".$log['orderNum']."'");
		$this->memberCls->addlog($member['uid'],1,2,$log["credits"],'兑换礼品','兑换礼品减少'.$log['score'].'积分',$member['score']);
	}
} 
?>
