...
//		$goodslist =  $this->mysql->getarr("select id,typeid,name,count,cost,bagcost,img,have_det from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid = ".$typeid." order by id desc  limit   ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."  ");
	...
	*/
	...
	function editshop(){
		...
		}elseif($is_orderbefore == 'is_orderbefore'){
	...
	*/
	function ordercontrol(){
		...
		if($dostring == 'domake'){ 
			...
			if($order['is_reback'] > 1 ) $this->message('退款处理');
			if($order['status'] > 3) $this->message('订单已取消');
			if($order['status'] == 2) $this->message('订单已发货');
			if($order['status'] == 3) $this->message('订单已完成');
			if($order['status'] ==0) $this->message('订单还未通过审核，不能受理');
			if($order['paytype'] == 1){
				if($order['paystatus'] == 0){
					$this->message('订单未支付，等待支付后确认制作');
				} 
			}
			if($order['re_back'] > 0){
				$this->message('订单申请退款中不能确认制作'); 
			}
			$udata['is_make'] = 1;
			$this->mysql->update(Mysite::$app->config['tablepre'].'order',$udata,"id='".$orderid."'");
			$ordCls = new orderclass();
			$ordCls->writewuliustatus($order['id'],4,$order['paytype']);
            ...
            $ordCls = new orderclass();
		    $ordCls->writewuliustatus($orderid,5,$order['paytype']);  //商家不接单  物流状态
	 	    if( $order['paytype'] == 1 &&  $order['paystatus'] == 1 ){
				$drawbacklog = new drawbacklog($this->mysql,$this->memberCls);
				$drawdata = array();
				$drawdata['allcost'] = $order['allcost'];
				$drawdata['orderid'] = $order['id'];
			...
			//对订单发货
			if($order['is_reback'] > 0 ) $this->message('订单申请退款');
			if($order['status'] == 0) $this->message('订单状态未审核');
			if($order['status'] == 2) $this->message('订单已发货');
			if($order['status'] ==  3) $this->message('订单已完成');
			if($order['status'] > 3) $this->message('订单已取消');
			if($order['is_make'] == 0) $this->message('订单未确认制作');
			if($order['is_make'] == 2) $this->message('订单已取消制作'); 
			$udata['status'] = 2;
			$udata['sendtime'] = time();
			$this->mysql->update(Mysite::$app->config['tablepre'].'order',$udata,"id='".$orderid."'"); 
			$ordCls = new orderclass();
			$ordCls->writewuliustatus($order['id'],6,$order['paytype']);
			$ordCls->noticesend($orderid); 
		}elseif($dostring == 'over'){
			if($order['is_reback'] > 0 ) $this->message('订单申请退款');
			if($order['status'] == 0) $this->message('订单状态未审核');
			if($order['status'] == 1) $this->message('订单未发货');
			if($order['status'] ==  3) $this->message('订单已完成');
			if($order['status'] > 3) $this->message('订单已取消');
			if($order['is_make'] == 0) $this->message('订单未确认制作');
			if($order['is_make'] == 2) $this->message('订单已取消制作');  
		...
		*/
		...
		if($shopopentype == 0){
			...
			if(strpos($userAgent,"iPhone") || strpos($userAgent,"iPad") || strpos($userAgent,"iPod")){
				...
				$ios_waiting =   Mysite::$app->config['ios_waiting'];
				if($source == 1 && $ios_waiting == true){
				    $tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom=1   order by sort asc");
				    $tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom!=1   order by sort asc"); 
				}else{
			        $tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom=1   and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
					$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom!=1   and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094)  order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
				} 
			}else if(strpos($userAgent,"Android")){
				$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()."  and is_recom=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
			    $tempcb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()."  and is_recom!=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
			}else{
				$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
				$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  and endtime > ".time()." and is_recom!=1  and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
		...
		foreach($list as $key=>$value){
            ...
            $value['mobilemodule'] =Mysite::$app->config['mobilemodule'];
...
//		$data['mobilemodule']=Mysite::$app->config['mobilemodule'];
		$this->success($shopdata);
	}
   ...
   function addresslist(){
	...
	function apppaydata(){
		...
		$alipaydata = array(
						...
						'wmrid'=>'',
						'support'=>0,
						'RSA_PRIVATE'=>'',
						...
						'SELLER'=>'375952873@qq.com',
						'PARTNER'=>'2088901767028754',
