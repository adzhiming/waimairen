		...
		*/
		...
		if($shopopentype == 0){
			...
			if(strpos($userAgent,"iPhone") || strpos($userAgent,"iPad") || strpos($userAgent,"iPod")){
				...
				$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()."  and is_recom=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
			    $tempcb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()."  and is_recom!=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
			}else{
				$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx."  and endtime > ".time()." and is_recom=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
				$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1  ".$tempwherexxx." and endtime > ".time()." and is_recom!=1  and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
...
/* 		$newdata['bagcost'] = intval(IFilter::act(IReq::get('bagcost')));
	...
	function getnewconfig(){
		...
		$config = new config('appset.php',hopedir,$this->mid,$type=1,Mysite::$app->config['conf_table']);   
   ...
   function addresslist(){
	...
	function apppaydata(){
		...
		if($type == 'acount'){//余额充值 
			...
			if(!empty($alipaychek)){  
				$dopaydata = array('mid'=>$this->mid,'type'=>'yhorder','upid'=>$orderid,'cost'=>$orderinfo['sjcost'],'source'=>0,'status'=>0,'addtime'=>time());//支付数据 
				$this->mysql->insert(Mysite::$app->config['tablepre'].'onlinelog',$dopaydata);
				$newid = $this->mysql->insertid(); 
	...
	function appacoutpay(){
		...
		if($this->checksession()){
			...
			if($type == 'order'){//支付订单 
				...
				$this->memberCls->addmemcostlog($this->mid,$orderinfo['uid'],$backinfo['username'],$backinfo['cost'],2,$orderinfo['sjcost'],$accost,"下单优惠买单",$backinfo['uid'],$backinfo['username']);
				$membertinfo=   $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."member where  uid ='".$backinfo['uid']."'   order by uid asc limit 0,1");
