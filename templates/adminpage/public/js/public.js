...
function adddaohang(){
  var html = '<tr class="s_out">';
  html += '<input type="hidden" name="id[]" value="">';
  html += '<td>&nbsp;&nbsp;</td>';
  html += '<td align="left" style="padding-left:10px;">';
  html += '<input type="text" name="typename[]" id="typename[]" class="skey form-control" />';
  html += '</td><td align="left" style="padding-left:10px;">';
  html += '<input type="text" name="typeurl[]" id="typeurl[]" class="skey form-control"/>';
  html += '</td><td>';
  html += '<input type="text" name="typeorder[]" id="typeorder[]" class="skey form-control" style=\'width:100px;\'/>';
  html += '</td><td><a onclick="$(this).parent().parent().remove();" href="javascript:" class="fa fa-trash" title="移除"></a></td></tr>';
  $("#type").append(html);
}
//全选操作
function checkall()
...
function modifypwd()
{
	var showcontent = '<form method="post" name="form1" action="'+siteurl+'/index.php/90001/adminpage/member/module/adminmodify/datatype/json" onsubmit="return subform(\'\',this);"><div>旧密码:<input type="password" name="oldpwd" value=""></div><div style="padding-top:10px;">新密码:<input type="password" name="pwd" value=""></div><div style="padding-top:10px;">确认密码:<input style="width: 118px" type="password" name="pwds" value=""></div><div style="padding-top:10px;padding-left:30px;"><input type="submit" value="确认提交" class="button"> </div> </form>';
	//alert(showcontent);
	art.dialog({
    id: 'KDf435',
    title: '修改账号密码',
