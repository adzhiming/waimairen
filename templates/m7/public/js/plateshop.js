  ...
  $(window).scroll(function(){
	...
	$('.onsell').bind("click", function() {   
	    if($(this).attr('have_det') == 1){
			addproduct($(this).attr('mid'),$(this).attr('goodsid'),$(this).attr('shopid'),1,$(this)); 
		}else{
			addonedish($(this).attr('mid'),$(this).attr('goodsid'),$(this).attr('shopid'),1,$(this)); 
		}
	}); 
...
//获取评价
function getPingjia(mid,page)
{
	///$('#Comloading').show();
 
  	 $('#det_commm').html('');
  	 var url = siteurl+"/index.php?mid='+mid+'&ctrl=order&action=commentshop"; 
...
//获取留言
function getliuyan(mid,page)
{
	 
  	$('#showaskid').html('');
  	 var url = siteurl+"/index.php?mid='+mid+'&ctrl=ask&action=newshopask";
...
//鍙戝竷鐣欒█
function AddMessage(mid)
{
	if($('#MContent').val() == $('#MContent').attr('data')|| $('#MContent').val()=='')
	{
		 diaerror('留言内容不能为空'); 
	}else{
	  var url = siteurl+'/index.php?mid='+mid+'&ctrl=ask&action=saveask&datatype=json'; 
...
//删除已道菜品
function removeonedish(mid,gid,tshopid,num){ 
	 $('#loading').show();
...
function freshcart(mid){  
	    url = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=smallcatding&shopid='+shopid+'&random=@random@';
	    url = url.replace('@random@', 1+Math.round(Math.random()*1000)); 
...
/*验证码*/
function gophone(mid){
	            var tempurl = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=phonecode&random=@random@&phone=@phone@';
   	 	         tempurl = tempurl.replace('@random@', 1+Math.round(Math.random()*1000)).replace('@phone@',$('#phone').val());
	            $.getScript(tempurl);  
}
function showsend(mid,phone,time){
    ...
    $('#uphone').parent().after('<div class="cart_out_show" id="showcode"><label > 验 证 码：</label><input id="phonecode" class="hc_order_input_box" type="text" value="" size="10" name="phonecode"><span class="hc_order_lists_span_color"> * </span><span class="hc_order_lists_span"></span></div>');        
     ...
     $('#dosendbtn').bind("click", function() {    
   	 	 var tempurl = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=setphone&random=@random@&phone=@phone@';
   	 	     tempurl = tempurl.replace('@random@', 1+Math.round(Math.random()*1000)).replace('@phone@',$('#phone').val());
	         $.getScript(tempurl);      
    });
