...
function loginjs(){
    var url = siteurl+"/index.php?mid='+mid+'&ctrl=member&action=ajaxlogin&random=@random@";  
...
function freshcart(mid){ 
	...
	if($('#checktj_newcart').html() == undefined){
	    url = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=smallcat&shopid='+shopid+'&random=@random@';
	    url = url.replace('@random@', 1+Math.round(Math.random()*1000)); 
...
function ajaxdoorder(mid){
	/*创建遮照层*/
	$('body').append('<div id="cat_zhe" class="cart_zhe"></div>');
	$('body').append('<div id="goosshow" class="cart_out_cat"></div>'); 
	var  urlnew = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=showcatax&random=@random@';
