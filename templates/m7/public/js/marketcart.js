...
function freshcart(mid){ 
	    ...
	    url = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=marketcart&shopid='+shopid+'&random=@random@';
	    url = url.replace('@random@', 1+Math.round(Math.random()*1000)); 
...
function ajaxdoorder(mid,tshopid){
$("#top").hide();
	/*创建遮照层*/   
	$('body').append('<div id="cat_zhe" class="cart_zhe"></div>');
	$('body').append('<div id="goosshow" class="cart_out_cat"></div>'); 
	var  urlnew = siteurl+'/index.php?mid='+mid+'&ctrl=site&action=showcatax&showtype=market&random=@random@';
