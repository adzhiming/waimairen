...
if($verify_result) {//验证成功
   ...
   $backinfog = mysql_fetch_assoc($info);
	...
	if(!empty($backinfog)){
			...
			if($backinfog['type']=='order'){ 
					...
					if($backinfog['source'] == 1 || $backinfog['source'] == 2){
							$inlink = $cfg['siteurl'].'/index.php?mid='.$mid.'&ctrl=wxsite&action=subshow&orderid='.$backinfog['upid'];
					}						
					$showdata['type'] = 'order';
				    ...
				    $inlink = $cfg['siteurl'].'/index.php?mid='.$mid.'&ctrl=wxsite&action=costlog';  // 支付成功后跳转到余额明细页面
					$showdata['type'] = 'aount';
					$showdata['info'] = array();
					$showdata['inlink'] = $inlink;  
			}elseif($backinfog['type']=='yhorder') {
				
				    $inlink = $cfg['siteurl'].'/index.php?mid='.$mid.'&ctrl=wxsite&action=subpayhui';  // 支付成功后跳转到余额明细页面
    ...
    $inlink = $cfg['siteurl'].'/index.php?mid='.$mid.'&ctrl=wxsite&action=index';
    $showdata['inlink'] = $inlink; 
}
?>
