...
class IUrl
{
			...
			foreach($string as $value)
			{
				if($i <= 3  )
				{
					if ($i==1) {
						...
						$tmpKey = self::UrlCtrlName;
					}
					if ($i==3) {
						$tmpKey = self::UrlActionName;
					}				
					$re[$tmpKey] = $value;
					$i ++ ;
					continue;
	...
	public static function creatUrlEx($mid,$url='')
	{
		if(strpos($url, '/') === 0) {
			return self::creatUrl($mid.$url);
		}
		else {
			return self::creatUrl($mid.'/'.$url);
		}
	...
	public static function beginUrl($url='')
	{
		...
		if( isset($urlArray[self::MidName]) )
		{
			IReq::set(self::MidName,$urlArray[self::MidName]);
		}
